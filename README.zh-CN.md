...
- 扫描任意本地项目目录。
- 读取 Markdown 任务文件。
- 通过 `harness.yaml` 配置项目名、输出目录、忽略目录、检查命令和工具注册表。
- 执行项目自定义 checks，例如单元测试、lint、build。
- 声明工具名称、用途、参数契约和命令模板，供后续 Agent Runtime 审查。
...
`tools` 目前用于声明和报告可用工具，不会自动执行；后续工具调用应先校验参数契约。
...
`run-report.md`
给人看的报告，包含项目扫描结果、项目工件、运行契约、事件、工具注册表、项目检查结果、风险和下一步。

`run-result.json`
给程序读取的结构化结果，包含项目名、扫描分类、项目工件、工具注册表、checks 结果和下一步。
...
- 当前不会自动修改代码。
- 当前不会自动创建 PR。
- 当前重点是项目扫描、工具契约声明、检查执行、报告生成、事件记录和轻量评测。
