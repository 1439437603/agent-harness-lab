...
Agent Harness Lab 是一个面向真实本地项目的 TDD-first Agent Harness。它不是聊天机器人，也不是完整自动写代码 Agent，而是一个让 Agent 更安全进入项目的工程化底座：先扫描项目、读取任务、执行配置好的检查命令，再生成可复查的报告和机器可读结果。

## 它解决什么问题

在让 AI Agent 直接改代码之前，项目通常需要先回答几个问题：

- 当前项目有哪些代码、文档、配置和证据材料？
- 这个任务的目标、风险和下一步是什么？
- 项目自己的测试、lint 或 build 能不能跑通？
- Agent 的运行过程有没有可追踪的事件记录？
- 输出能不能既给人看，也给程序或 CI 继续处理？

Agent Harness Lab 解决的就是这些“进入项目之前”和“运行之后可复盘”的问题。
...
`run-report.md`
给人看的报告，包含项目扫描结果、证据文件、运行契约、事件、项目检查结果、风险和下一步。

`run-result.json`
给程序读取的结构化结果，包含项目名、扫描分类、证据文件、checks 结果和下一步。
